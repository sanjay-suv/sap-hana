...
resource "null_resource" "ansible_playbook" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Test winrm with python3
      "cd sap-hana",
      "git checkout nancyc-winrm-fix-test",
      "cd ~",
      var.options.ansible_execution ? "ansible-playbook -i hosts ~/sap-hana/deploy/v2/ansible/sap_playbook_test.yml" : "ansible-playbook --version"
    ]
  }
}
